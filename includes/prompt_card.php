...
?>
    <div class="group relative bg-white rounded-3xl border border-slate-200 hover:border-primary-400 hover:shadow-2xl hover:shadow-primary-900/10 transition-all duration-300 flex flex-col overflow-hidden h-full">
        ...
        <div class="relative z-10 p-5 flex flex-col h-full">
            <div class="flex justify-between items-start mb-4">
                <div class="flex flex-wrap gap-1.5">
                    <?php if (isset($prompt['category_slug'])): ?>
                        <a href="<?php echo APP_URL_BASE; ?>/prompts/<?php echo esc($prompt['category_slug']); ?>" class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold bg-primary-50 text-primary-700 uppercase tracking-widest border border-primary-100/50 hover:bg-primary-600 hover:text-white hover:border-primary-600 transition-all duration-300">
            ...
            <div class="flex-grow">
                <a href="<?php echo APP_URL_BASE; ?>/prompt.php?id=<?php echo $prompt['id'] . '-' . $prompt['slug']; ?>" class="block mb-1">
                ...
                <div class="flex items-center gap-1.5 mb-3">
                    ...
                    <a href="<?php echo APP_URL_BASE; ?>/u/<?php echo esc($prompt['author_slug'] ?? slugify($prompt['author_name'])); ?>" class="text-[10px] font-bold text-slate-400 hover:text-primary-600 transition-colors uppercase tracking-widest">
            ...
            <div class="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <?php if (is_logged_in() && $prompt['user_id'] == get_current_user_id()): ?>
                        <a href="<?php echo APP_URL_BASE; ?>/prompt_edit.php?id=<?php echo $prompt['id']; ?>" class="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-all" title="Edit">
                    ...
                    <?php endif; ?>
                    <a href="<?php echo APP_URL_BASE; ?>/prompt.php?id=<?php echo $prompt['id'] . '-' . $prompt['slug']; ?>" class="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-xl transition-all" title="View Full Prompt">
