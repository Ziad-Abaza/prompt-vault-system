...
$canonical_url = rtrim(Env::get('APP_URL', ''), '/') . '/prompts/tag/' . $tag['slug'];

// Pagination URLs for rel="prev/next"
$base_pagination_url = rtrim(Env::get('APP_URL', ''), '/') . '/prompts/tag/' . $tag['slug'];
...
$breadcrumbs = [
    ...
    ['name' => 'Tags', 'url' => 'public_prompts.php'],
    ['name' => $tag['name'], 'url' => "prompts/tag/{$tag['slug']}"]
];
