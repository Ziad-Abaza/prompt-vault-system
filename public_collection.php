...
$canonical_url = rtrim(Env::get('APP_URL', ''), '/') . '/collections/' . $collection['slug'];

// Pagination URLs for rel="prev/next"
$base_pagination_url = rtrim(Env::get('APP_URL', ''), '/') . '/collections/' . $collection['slug'];
...
$breadcrumbs = [
    ...
    ['name' => 'Collections', 'url' => 'public_prompts.php'],
...
<div class="max-w-7xl mx-auto">
    ...
    <nav class="flex mb-8" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest">
            ...
            <li>
                <a href="public_prompts.php" class="text-slate-400 hover:text-primary-600 transition-colors">Collections</a>
            </li>
            <li>
                <svg class="h-3 w-3 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7" />
        ...
        <?php if ($total_pages > 1): ?>
            <div class="mt-12 mb-8 flex justify-center">
                <nav class="flex items-center space-x-2">
                    <?php if ($page > 1): ?>
                        <a href="collections/<?php echo $collection['slug']; ?>?page=<?php echo $page - 1; ?>" class="px-5 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 font-bold text-[10px] uppercase tracking-wider hover:bg-slate-50 transition-all">Previous</a>
                    <?php endif; ?>
                    ...
                    <?php if ($page < $total_pages): ?>
                        <a href="collections/<?php echo $collection['slug']; ?>?page=<?php echo $page + 1; ?>" class="px-5 py-2.5 rounded-xl bg-slate-900 text-white font-bold text-[10px] uppercase tracking-wider hover:bg-primary-600 transition-all shadow-lg shadow-slate-900/20">Next Page</a>
                    <?php endif; ?>
                </nav>
            </div>
        <?php endif; ?>
