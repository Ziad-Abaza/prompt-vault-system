...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($validator->is_valid()) {
        ...
        try {
            ...
            redirect(APP_URL_BASE . '/categories.php');
        } catch (Exception $e) {
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-4">
            <div class="bg-white rounded-[2rem] border border-slate-200 shadow-xl shadow-slate-200/40 overflow-hidden sticky top-8">
                ...
                <form action="<?php echo APP_URL_BASE; ?>/categories.php" method="POST" class="p-8 space-y-6">
                    <?php echo csrf_input(); ?>
                    <?php if ($edit_cat): ?>
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <?php if ($edit_cat): ?>
                            <a href="<?php echo APP_URL_BASE; ?>/categories.php" class="w-full py-4 bg-white border border-slate-200 text-slate-500 font-bold rounded-2xl hover:bg-slate-50 hover:text-slate-700 text-center transition-all text-xs uppercase tracking-widest">
                                Discard Changes
                            </a>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="category-card group bg-white rounded-3xl border border-slate-200 p-6 hover:border-primary-300 hover:shadow-xl hover:shadow-primary-900/5 transition-all duration-300" data-name="<?php echo strtolower(esc($cat['name'])); ?>">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                                <a href="<?php echo APP_URL_BASE; ?>/index.php?category_id=<?php echo $cat['id']; ?>" class="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-xl transition-all" title="View Prompts">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                </a>
                                <a href="<?php echo APP_URL_BASE; ?>/categories.php?edit=<?php echo $cat['id']; ?>" class="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-all" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <a href="<?php echo APP_URL_BASE; ?>/categories.php?delete=<?php echo $cat['id']; ?>" onclick="return confirm('Delete this category? Prompts will become Uncategorized.');" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all" title="Delete">
